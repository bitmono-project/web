...
// Crackmes RSS — the feed itself is generated by the API (challenge data lives in the DB), served
// here at a clean URL to mirror /blog/rss.xml. Autodiscovery <link> is injected site-wide below.
app.get('/crackmes/rss.xml', async (req, res) => {
  try {
    const r = await fetch(`${apiUrl}/api/feed/challenges.rss`);
    res.status(r.status).set('Content-Type', 'application/rss+xml; charset=utf-8')
      .set('Cache-Control', 'public, max-age=3600').send(await r.text());
  } catch {
    res.status(502).end();
  }
});
...
app.use(async (req, res) => {
  try {
    ...
    res.status(head.status ?? 200)
      ...
      .send(injectHead(template, head, { gscToken, rssUrl: `${origin}/blog/rss.xml`, crackmesRssUrl: `${origin}/crackmes/rss.xml` }));
  } catch {
